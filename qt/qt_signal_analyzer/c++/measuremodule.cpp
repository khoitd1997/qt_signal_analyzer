...
MeasureModule::MeasureModule(QList<QVector<QPointF> *> &allData_, QReadWriteLock *allDataLock)
    : allDataLock_(allDataLock), allData_(allData_) {
  singleton = this;
}

QObject *MeasureModule::singletonProvider(QQmlEngine *engine, QJSEngine *scriptEngine) {
  Q_UNUSED(engine)
...
void MeasureModule::setGuiSource(QObject *measureGUI) {
  connect(measureGUI,
          SIGNAL(measureChanged(int, int, int, bool)),
          this,
          SLOT(switchMeasure(int, int, int, bool)));
  connect(this, SIGNAL(measureFinished()), measureGUI, SLOT(changeMeasureResults()));
}
...
QVector<QPointF>::reverse_iterator MeasureModule::findFirstPointInRange(
  ...
  do {
    ...
    if (trialCount > kMaxPeriodCalcTrial) { return res; }
...
qreal MeasureModule::getPeriod(const int sourceIndex) {
  ...
  auto startPoint =
      findFirstPointInRange(allData_[sourceIndex]->rbegin(),
                            allData_[sourceIndex]->rend(),
                            range,
                            kMidPointRangeInc,
                            [maxY, range](const QPointF &p) {
                              return ((p.y() < maxY + range) && (p.y() > maxY - range));
                            });
  if (allData_[sourceIndex]->rend() == startPoint) { return -1; }
  range = kDefaultMidPointRange;

  auto midPoint1 =
      findFirstPointInRange(startPoint,
                            allData_[sourceIndex]->rend(),
                            range,
                            kMidPointRangeInc,
  ...
  if (allData_[sourceIndex]->rend() == midPoint1) { return -1; }
  range = kDefaultMidPointRange;

  auto endMidPoint1 =
      std::find_if(midPoint1, allData_[sourceIndex]->rend(), [midPointY, range](const QPointF &p) {
        return (p.y() < midPointY - range);
      });
  if (endMidPoint1 == allData_[sourceIndex]->rend()) { return -1; }
  range = kDefaultMidPointRange;

  auto midPoint2 =
      findFirstPointInRange(endMidPoint1 + 1,
                            allData_[sourceIndex]->rend(),
                            range,
                            kMidPointRangeInc,
                            [midPointY, range](const QPointF &p) {
  ...
  if (allData_[sourceIndex]->rend() == midPoint2) { return -1; }

  return (midPoint1->x() - midPoint2->x()) * 2;
}

void MeasureModule::switchMeasure(const int  slotIndex,
                                  const int  newType,
                                  const int  newSource,
